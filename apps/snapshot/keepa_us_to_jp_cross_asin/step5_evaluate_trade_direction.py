...
#
# 【このプログラムの役割】
# Step4でブランド判定が完了した（wakarunda=D または -）ASINを対象に、
# 輸出（日本→米国/カナダ）または輸入（米国/カナダ→日本）の
# どちらが採算に合うかを計算して判定する。
# 現在の為替レートと輸送費テーブル（Excelファイル）を元に
# 目標販売価格を算出し、実際の現地価格と比較する。
# 結果（輸出/輸入/不可）と目標価格・送料をDBへ書き込む。
#
import os
import sys
...
# ==========================================
# 1件のASINについて、輸出・輸入それぞれの採算を計算する。
# 日本・米国・カナダの価格と商品の寸法・重量から送料を算出し、
# 目標マージンを確保できるかどうかを判定する。
# 結果は辞書形式で返し、「輸出」「輸入」「不可」などの文字列と
# 各種目標価格・送料を含む。
def evaluate_profitability(
    ...
    # 結果格納用の辞書を初期化する（計算できない場合はNoneのまま返す）
    res = {
    ...
    # 💡 必須データチェック
    # 寸法・重量が揃っていなければ送料計算ができないため処理をスキップする
    ...
    # 各市場の価格を円換算する（US・CA価格も為替レートで円に変換）
    ...
    # 寸法から容積重量を計算し、実重量と大きい方を課金対象重量とする
    ...
    # 送料テーブル（Excelファイル）から各方向の送料を計算する
    try:
    ...
    # US市場の評価
    # ========================================
    # JP価格とUS価格・送料を比較して、輸出/輸入/不可を判定する
    eval_us = None
    ...
    if us_exist == 1:
        if us_p:
            ...
            if jp_p is None:
                ...
                # US価格がなくJP価格がある場合：ライバル不在の輸出として扱う
                total_cost_us = jp_p + (cost_us or 0)
                us_target_price_y = total_cost_us / (1 - DEFAULT_FEE_RATE - TARGET_MARGIN_US)
                us_target_price_d = us_target_price_y / exchange_rate_us
                eval_us = "輸出(ライバル不在)"
    ...
    # CA市場の評価
    # ========================================
    # JP価格とCA価格・送料を比較して、輸出/輸入/不可を判定する
    eval_ca = None
    ...
    if ca_exist == 1:
        if ca_p:
            ...
            if jp_p is None:
                ...
                # CA価格がなくJP価格がある場合：ライバル不在の輸出として扱う
                total_cost_ca = jp_p + (cost_ca or 0)
                ca_target_price_y = total_cost_ca / (1 - DEFAULT_FEE_RATE - TARGET_MARGIN_CA)
                ca_target_price_d = ca_target_price_y / exchange_rate_ca
                eval_ca = "輸出(ライバル不在)"
    ...
    # JP目標価格の最終決定 (最小値を採用)
    # ========================================
    # 輸入評価がUS・CA両方ある場合は、より安い方（仕入れやすい方）を採用する
...
# ==========================================
# プログラムの起動点。
# 為替レートを取得した後、評価対象のASINをDBから全件抽出する。
# 1件ずつ採算計算を行い、結果を1000件まとめてDBへ一括更新する。
def main():
    # 現在の為替レート（USD・CAD）を取得する
    print("為替レートを取得しています...")
    ...
    # 評価対象のASINをDBから取得する（価格・寸法・重量を含む）
    sql_select = """
        SELECT
            asin,
            jp_lowest_price_y,
    ...
    for row in rows:
        ...
        # 1000件に達したらDBへ一括UPDATEしてリストをリセットする
