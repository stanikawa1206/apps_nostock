#   3. 日米加の価格比較・輸出入判定プログラム (File 3)
#   このプログラムは、特定のASIN（商品コード）について日本・アメリカ・カナダの価格を比較し、その商品が「輸出」と「輸入」のどちらに向いているかを判定する分析ツールです。  
#   3カ国のAmazonから商品の最安値と出品者情報（カート取得の有無、評価数、FBA/FBM、出荷日数など）を取得します。  
#   各国の価格差を比較し、どこで仕入れるのが最も安いかをもとに「輸出向き」「輸入向き」「双方向の可能性あり」「価格差なし」の判定を下します。  
#   仕入先の候補から条件の悪いセラー（評価10件未満や高評価80%未満など）を足切りし、安全な候補だけを価格が安い順にリストアップして画面に表示します。  
