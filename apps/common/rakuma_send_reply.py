"""
/messages画面の「送信」ボタンから、ラクマの取引ページへ実際に返信を送信する処理。

Webアプリ本体（D:\\apps_resale\\furima\\webapp\\messages_blueprint.py）から呼び出される。
DOM操作（本文入力欄・送信ボタンの特定、送信前の再確認、実送信結果の確認）は
apps/etc/rakuma_purchase.py の send_chat_message() に実装済みのものをそのまま使う
（本モジュールでは新たにセレクタ・判定ロジックを実装しない）。

既存のrakuma_purchase.pyが使っているChromeデバッグセッション（ポート9223・
...
from apps.etc.rakuma_purchase import (  # noqa: E402
    DEBUG_PORT,
...
def send_rakuma_reply(vendor_item_id: str, expected_last_message_no, reply_text: str) -> dict:
    """
    expected_last_message_no: /messages画面表示時点の最新メッセージのmessage_no
    （ラクマにはメルカリのような安定した一意message_idが無いため、
    件数ベースの位置で新着有無を判定する）。
