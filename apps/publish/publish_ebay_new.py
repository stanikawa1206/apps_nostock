...
def parse_detail_rakuma(page, url: str, preset: str, vendor_name: str) -> Dict[str, Any]:
    ...
    () => {
        ...
        return keys.filter(x =>
            x.toLowerCase().includes("item") ||
            x.toLowerCase().includes("nuxt") ||
            x.toLowerCase().includes("apollo")
        )
    }
    """)
    ...
    created_at_unix = page.evaluate("""
    () => {
        return window.__NUXT__?.data?.[0]?.data?.created
    }
    """)

    updated_at_unix = page.evaluate("""
    () => {
        return window.__NUXT__?.data?.[0]?.data?.updated
    }
    """)

    print(created_at_unix)
    ...
    if created_at_unix:
        vendor_created_at = datetime.fromtimestamp(created_at_unix)

    if updated_at_unix:
        vendor_updated_at = datetime.fromtimestamp(updated_at_unix)

    print(vendor_created_at)
    print(vendor_updated_at)
    ...
    title = page.locator("h1.item__name").inner_text()
    title = title.replace("\xa0", " ")

    price_text = page.locator("span.item__price").inner_text()
    price = int(
    ...
    description = page.locator(
        "div.item__description__line-limited"
    ).first.inner_text()
    ...
    review_spans = page.locator(
        "div.review-list span"
    ).all_inner_texts()

    rating_count = sum(
        int(x.replace(",", ""))
        for x in review_spans
    )

    shipping_days = shop.get_attribute(
        "data-rat-cp-shipping_date_estimate"
    )
    shipping_region = shop.get_attribute(
        "data-rat-cp-shipping_from"
    )
    item_condition = shop.get_attribute(
...
def heavy_check_detail(
    ...
    try:
        ...
        if vendor_name == "メルカリshops":
            ...
            rec = parse_detail_rakuma(page, item_url, preset, vendor_name)
        else:
            # 通常メルカリ
            ...
            while not stop_all:
                ...
                while not stop_all:
                    ...
                    if vendor_name == "メルカリshops":
                        ...
                        item_url = f"https://fril.jp/item/{sku}"
                    else:
